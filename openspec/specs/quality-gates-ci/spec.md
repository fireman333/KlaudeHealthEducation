...
GitHub Actions CI 兩道 hard gate，把 `desktop-sidebar` capability Req 10（client JS bundle 大小上限）+ Req 11（Lighthouse 三項分數下限）從 maintainer 手動人審轉成自動化、fail-loud 的 PR check。每道 gate 設計成 (a) PR + push 到 main 都觸發、(b) 跟 `deploy.yml` 解耦不互卡、(c) 本機可重現避免 debug 拖拍。閾值寫進 spec 不寫進 README，PR 不可 silently bump 數字、需開新 OpenSpec change 並更新本 capability。
...
- **WHEN** Lighthouse runs and reports Accessibility = 96, SEO = 100, Performance = 65
- **THEN** the Lighthouse step SHALL succeed
- **AND** the CI log SHALL report all three scores

#### Scenario: Accessibility below 95 fails
- **WHEN** Lighthouse reports Accessibility = 93 while SEO = 100 and Performance = 65
...
- **AND** the section SHALL state the Lighthouse thresholds (A11y/SEO ≥ 95, Performance ≥ 50)
...
- **THEN** the Quality gates section SHALL reference the file paths where thresholds are configured (e.g., `lighthouserc.json`, `.size-limit.cjs`)
