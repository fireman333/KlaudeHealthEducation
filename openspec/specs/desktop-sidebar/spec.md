...
Desktop-only sidebar 系統提供文章閱讀頁面的次要導覽：作者釘選文章（Pinned）、整站時間軸索引（Timeline）、以及 Phase 2 解鎖的熱門文章區（Popular）。於 ≥1024 px 桌機 viewport 顯示為右欄、<1024 px 改用 CSS-only `<details>/<summary>` 摺疊；嚴守 DESIGN.md 8 條 chrome rules（light only / 無 shadow / 1px hairline / 無動畫例外允許 native `<details>` toggle / 240 px max width / sage 配色）。MVP 採零 client JS（純 server-rendered Astro component），未來 sidebar 新增 chunk 受 5 KB gzipped delta 限制。
...
**Note on Performance threshold revision**: the original wording was `≥ 95` for all three categories. Apply phase of `add-quality-gates-ci` measured actual scores across local + CI:
